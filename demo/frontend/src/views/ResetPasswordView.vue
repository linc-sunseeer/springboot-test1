    ...
    <div class="relative w-full max-w-md bg-white rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/60 overflow-hidden z-10">
      <div class="bg-gradient-to-br from-orange-500 to-orange-400 p-8 text-center relative overflow-hidden flex flex-col items-center">
        ...
        <div class="w-16 h-16 bg-white rounded-full p-2 shadow-lg mb-3 relative z-10 flex items-center justify-center">
          <span class="text-4xl font-bold text-bento-orange">P</span>
        </div>
        <h2 class="text-2xl font-bold text-white tracking-wide relative z-10">パスワード再設定</h2>
        <p class="text-orange-50 mt-2 text-sm font-medium relative z-10">メール確認が完了しました。新しいパスワードを設定してください。</p>
      </div>
      ...
      <div class="p-8">
        <div class="mb-6 rounded-2xl border border-orange-100 bg-orange-50/70 px-4 py-3 text-xs leading-6 text-orange-700 font-medium">
          このページはメール内リンクから開かれています。新しいパスワードを設定すると、次回以降はそのパスワードでログインできます。
        </div>

        <form class="space-y-5" @submit.prevent="submitReset">
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1.5">新しいパスワード</label>
            <div class="relative">
              <input
                v-model="password"
          ...
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1.5">新しいパスワード（確認）</label>
            <input
...
async function submitReset() {
  if (!token.value.trim()) {
    errorMessage.value = '認証リンクが無効です。メールを開き直してください。'
    return
  }
  ...
  try {
    ...
    message.value = 'パスワードを再設定しました。ログイン画面へ移動します。'
    errorMessage.value = ''
    setTimeout(() => router.push('/login'), 1200)
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : 'パスワード再設定に失敗しました。'
  } finally {
