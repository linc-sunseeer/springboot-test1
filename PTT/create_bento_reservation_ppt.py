...
OUT = BASE_DIR / "弁当予約管理システム_新人研修成果報告_改訂版_安全対策強化.pptx"
ROOT_COPY = BASE_DIR.parent / "弁当予約管理システム_新人研修成果報告_改訂版_安全対策強化.pptx"
...
for i, item in enumerate(agenda):
    ...
    x = 2.0 + col * 11.8
    y = 2.7 + row * 1.25
    pill(s, x, y, 1.0, 0.6, f"{i + 1}", BLUE if col == 0 else GREEN, 10)
    text(s, x + 1.35, y + 0.08, 9.7, 0.38, item, 14, True, TEXT)

# 3
s = slide(prs, 2, "システム概要", "実装済みコードに基づく概要")
...
# 4
s = slide(prs, 3, "機能一覧", "Controller / Router から確認できる実装範囲")
...
# 5
s = slide(prs, 4, "開発環境＆開発言語", "pom.xml / package.json / compose.yaml に基づく")
...
# 6
s = slide(prs, 5, "開発スケジュール", "前輩資料の形式に合わせた工程整理")
schedule = [
    ("4月", "要件整理・DB設計", GREEN),
    ("5月", "API設計・認証実装", BLUE),
    ("6月", "予約・管理画面実装", ORANGE),
...
# 7
s = slide(prs, 6, "基本設計-アーキテクチャ図", "Vue SPA + REST API + MyBatis-Plus")
...
# 8
s = slide(prs, 7, "基本設計-データベース", "schema.sql の主要テーブル")
...
# 9
s = slide(prs, 8, "基本設計-画面遷移図", "frontend/src/router/index.js に基づく")
...
# 11
s = slide(prs, 10, "セキュリティ対策-認証とROLE判定", "Spring Security / JWT / BearerTokenAuthenticationFilter")
steps = [
    ("1", "ログイン", BLUE),
    ("2", "JWT発行", GREEN),
    ("3", "Bearer Token送信", ORANGE),
    ("4", "Token検証", BLUE),
    ("5", "ROLE付与", GREEN),
    ("6", "API権限判定", RED),
]
for i, (num, label, color) in enumerate(steps):
    x = 1.1 + i * 4.1
    pill(s, x, 2.75, 0.75, 0.65, num, color, 10)
    pill(s, x + 0.85, 2.75, 2.8, 0.65, label, color, 10)
    if i < len(steps) - 1:
        arrow(s, x + 3.7, 3.08, x + 4.0, 3.08)
card(s, 1.1, 4.55, 7.7, 4.0, "JWTに入れる情報", [
    "subject：ユーザーID / 管理者ID",
    "userType：USER または ADMIN",
    "name：表示用の名前",
    "expiration：有効期限",
], BLUE, 11)
card(s, 9.45, 4.55, 7.7, 4.0, "FilterでのROLE変換", [
    "Authorization: Bearer xxx を取得",
    "署名と有効期限を検証",
    "ADMINなら ROLE_ADMIN",
    "それ以外は ROLE_USER",
], GREEN, 11)
card(s, 17.8, 4.55, 7.7, 4.0, "URLごとの権限制御", [
    "/api/auth/**：ログイン前でも可",
    "/api/user/**：USER / ADMIN",
    "/api/admin/**：ADMINのみ",
    "権限なしの場合はアクセス不可",
], RED, 11)
add_table(s, 2.2, 9.6, 22.0, 2.2, [
    ["API", "必要な権限", "用途"],
    ["/api/auth/**", "不要", "ログイン・登録・メール認証"],
    ["/api/user/**", "ROLE_USER / ROLE_ADMIN", "メニュー閲覧・予約・マイページ"],
    ["/api/admin/**", "ROLE_ADMIN", "管理画面・集計・設定"],
], [7.0, 6.0, 9.0], 10)

# 12
s = slide(prs, 11, "セキュリティ対策-データ保護", "パスワード・メール認証・管理操作")
card(s, 1.2, 2.55, 11.5, 3.5, "パスワード保護", [
    "DBには平文パスワードを保存しない",
    "BCryptPasswordEncoder でハッシュ化",
    "ログイン時は matches() で照合",
], BLUE, 12)
card(s, 14.0, 2.55, 11.5, 3.5, "メール認証", [
    "登録直後は email_verified = false",
    "確認リンク完了後にログイン可能",
    "未認証ユーザーのログインを防止",
], GREEN, 12)
card(s, 1.2, 7.1, 11.5, 3.9, "トークン管理", [
    "email_tokens テーブルで管理",
    "token_hash を保存し、有効期限を設定",
    "used_at で使用済みを判定",
    "パスワード再設定にも利用",
], ORANGE, 12)
card(s, 14.0, 7.1, 11.5, 3.9, "管理者操作の安全性", [
    "ユーザー削除時に管理者パスワードを再確認",
    "画像アップロードはサイズ制限を設定",
    "CSV出力時に Content-Type / nosniff を指定",
    "JWT secret は環境変数で変更可能",
], RED, 12)

# 13
s = slide(prs, 12, "テスト仕様書", "src/test/java にあるテストを整理")
...
# 14
s = slide(prs, 13, "苦労した点", "実装内容から見た技術課題")
card(s, 1.4, 2.75, 11.6, 7.4, "バックエンド", [
    "Spring Security と ROLE 判定の整理",
...
# 16
s = slide(prs, 15, "テスト-要件実現確認", "要件定義の項目に対する実装状況")
add_table(s, 0.9, 2.45, 24.9, 7.4, [
    ["要件項目", "実現状況", "確認内容"],
    ["ユーザー登録・ログイン", "実現済み", "登録、JWTログイン、メール認証"],
    ["弁当メニュー閲覧", "実現済み", "本日メニュー、週間メニュー、詳細画面"],
    ["予約・カート予約", "実現済み", "単品予約、複数メニューの一括予約"],
    ["支払・注文状態管理", "実現済み", "支払申請、管理者承認、注文状態更新"],
    ["予約集計", "実現済み", "日次・月次集計、CSV出力"],
    ["メニュー管理", "実現済み", "登録、更新、削除、画像アップロード"],
    ["ユーザー管理", "実現済み", "ユーザー一覧、予約履歴、削除"],
    ["通知機能", "一部実現済み", "メール認証、再設定、リマインダー、成団通知"],
], [6.0, 4.0, 14.9], 9)
text(s, 2.0, 10.7, 22.8, 0.65, "主要要件は実装済み。外部決済や画面操作テストの強化は今後の課題として整理。", 15, True, NAVY, PP_ALIGN.CENTER)

# 17
s = slide(prs, 16, "独自開発機能", "要件に追加して実装した工夫")
card(s, 1.2, 2.55, 11.5, 3.7, "成団管理", [
    "最小予約人数を system_settings で管理",
    "予約数に応じて成団確定メッセージを返却",
    "管理者が全件確定・全件キャンセルを実行",
], GREEN, 12)
card(s, 14.0, 2.55, 11.5, 3.7, "食事設定", [
    "ユーザーごとの目標カロリーを保存",
    "アレルゲン設定をプロフィールで管理",
    "メニュー側にも熱量・アレルゲン項目を保持",
], BLUE, 12)
card(s, 1.2, 7.3, 11.5, 3.7, "メール連携", [
    "メール認証",
    "パスワード再設定",
    "予約リマインダー・成団通知の拡張余地",
], ORANGE, 12)
card(s, 14.0, 7.3, 11.5, 3.7, "管理者向け運用機能", [
    "日次・月次CSVエクスポート",
    "返金理由付きの返金処理",
    "画像アップロードによるメニュー管理",
], RED, 12)
