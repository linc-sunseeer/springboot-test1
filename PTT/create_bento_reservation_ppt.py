...
# 11
s = slide(prs, 10, "セキュリティ対策", "SecurityConfig / Service 実装に基づく")
card(s, 1.2, 2.55, 11.5, 3.8, "認証・認可", [
    "JWT Bearer Token によるステートレス認証",
    "/api/admin/** は ADMIN 権限のみ許可",
    "/api/user/** は USER / ADMIN 権限で制御",
], BLUE, 12)
card(s, 14.0, 2.55, 11.5, 3.8, "パスワード保護", [
    "BCryptPasswordEncoder でハッシュ化",
    "パスワード再設定はトークン方式",
    "管理者削除操作は管理者パスワード確認",
], GREEN, 12)
card(s, 1.2, 7.35, 11.5, 3.8, "メール・トークン", [
    "メール認証で登録ユーザーを確認",
    "token_hash をDB保存し、有効期限を管理",
    "使用済みトークンを used_at で管理",
], ORANGE, 12)
card(s, 14.0, 7.35, 11.5, 3.8, "API・ファイル", [
    "CSRF無効化 + JWT 前提のAPI設計",
    "画像アップロードは multipart サイズ制限",
    "CSV出力は Content-Type / nosniff を設定",
], RED, 12)

# 12
s = slide(prs, 11, "テスト仕様書", "src/test/java にあるテストを整理")
...
# 13
s = slide(prs, 12, "苦労した点", "実装内容から見た技術課題")
...
# 15
s = slide(prs, 14, "テスト-要件実現確認", "要件定義の項目に対する実装状況")
...
text(s, 2.0, 10.7, 22.8, 0.65, "主要要件は実装済み。外部決済やE2Eテストは今後の課題として整理。", 15, True, NAVY, PP_ALIGN.CENTER)

# 16
s = slide(prs, 15, "独自開発機能", "要件に追加して実装した工夫")
