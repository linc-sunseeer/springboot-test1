"""Generate updated PPT for 弁当予約管理システム 新人研修成果報告."""
...
OUT = BASE_DIR / "弁当予約管理システム_新人研修成果報告_改訂版.pptx"
ROOT_COPY = BASE_DIR.parent / "弁当予約管理システム_新人研修成果報告_改訂版.pptx"
...
# 3 - System Overview
s = slide(prs, 2, "システム概要", "開発目的と対象ユーザー")
...
text(s, 2.0, 11.3, 22.8, 0.8, "予約・集計・通知をAPI中心に扱う構成", 17, True, NAVY, PP_ALIGN.CENTER)

# 4 - Schedule
s = slide(prs, 3, "開発スケジュール", "約3ヶ月（2026年4月〜7月）")
schedule = [
    ("4月", "要件定義・設計", GREEN),
    ("5月", "環境構築・基本機能", BLUE),
    ("6月", "詳細機能・テスト", ORANGE),
...
for i, (month, task, color) in enumerate(schedule):
    ...
    text(s, x + 0.25, 4.5, 4.0, 0.8, task, 14, True, TEXT, PP_ALIGN.CENTER)
    if i < 3:
        arrow(s, x + 4.65, 3.6, x + 5.75, 3.6)
add_table(s, 2.0, 7.0, 22.4, 3.5, [
    ["期間", "作業内容"],
    ["4月", "要件定義・設計"],
    ["5月", "環境構築・基本機能"],
    ["6月", "詳細機能実装・テスト"],
...
# 5 - Tech Stack
s = slide(prs, 4, "開発環境＆技術スタック", "pom.xml / package.json に基づく")
...
# 6 - User Features
s = slide(prs, 5, "ユーザー機能一覧", "一般ユーザーができること")
card(s, 1.1, 2.6, 7.7, 8.7, "認証・アカウント", [
    "ユーザー登録",
    "メール認証",
    "JWTログイン",
    "パスワード再設定",
...
# 7 - Admin Features
s = slide(prs, 6, "管理者機能一覧", "管理者ができること")
card(s, 1.1, 2.6, 7.7, 8.7, "ダッシュボード", [
    "予約状況の統計",
    "日別・月別集計",
    "CSVエクスポート",
], GREEN, 12)
card(s, 9.45, 2.6, 7.7, 8.7, "メニュー管理", [
    "メニュー登録・編集・削除",
    "画像アップロード",
    "提供日・価格設定",
], BLUE, 12)
card(s, 17.8, 2.6, 7.7, 8.7, "運用管理", [
    "ユーザー一覧・削除",
    "予約確定・キャンセル",
    "返金処理",
    "システム設定",
...
# 8 - Screen Flow
s = slide(prs, 7, "システム画面紹介", "主要画面とできること")
...
# 9 - Database
s = slide(prs, 8, "データベース設計", "6つの主要テーブル")
...
# 10 - Security & Good Points
s = slide(prs, 9, "工夫した点", "セキュリティと使いやすさ")
card(s, 1.2, 2.55, 11.5, 3.8, "セキュリティ", [
    "JWT Bearer Token による認証",
    "パスワードはハッシュ化して保存",
    "メール認証で本人確認",
], BLUE, 12)
card(s, 14.0, 2.55, 11.5, 3.8, "使いやすい設計", [
    "前後端分離で画面が速い",
    "カート機能で複数予約できる",
    "画像アップロードでメニューが見やすい",
], GREEN, 12)
card(s, 1.2, 7.35, 11.5, 3.8, "テスト", [
    "単体テスト 18 件実施",
    "H2 データベースでテスト環境",
    "Controller 層のAPI動作も確認",
], ORANGE, 12)
card(s, 14.0, 7.35, 11.5, 3.8, "独自機能", [
    "成団管理（最小予約人数）",
    "食事設定（カロリー・アレルゲン）",
    "メール通知（認証・再設定・リマインダー）",
], RED, 12)
...
# 13 - Summary
s = slide(prs, 12, "まとめ", "3ヶ月で完成したシステム")
card(s, 1.4, 2.75, 11.6, 4.0, "完成したもの", [
    "ユーザー機能 + 管理者機能を実装",
    "セキュリティ・テストも対応",
    "前後端分離のモダンな設計",
], GREEN, 13)
card(s, 14.0, 2.75, 11.6, 4.0, "学んだこと", [
    "Spring Boot の使い方",
    "Vue.js の基本",
    "チーム開発の流れ",
], BLUE, 13)
text(s, 2.0, 9.0, 22.8, 0.8, "主要要件は実装済み。外部決済やE2Eテストは今後の課題。", 17, True, NAVY, PP_ALIGN.CENTER)
